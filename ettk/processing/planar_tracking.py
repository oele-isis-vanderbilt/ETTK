...
class PlanarResult:
    ...
    def draw_aruco_markers(self, img: np.ndarray, corners: np.ndarray, ids: np.ndarray):
        ...
        if np.all(ids is not None):  # If there are markers found by detector
            for i in range(0, len(ids)):  # Iterate in markers
                ...
                cv2.drawFrameAxes(
                    img, MATRIX_COEFFICIENTS, DISTORTION_COEFFICIENTS, rvec, tvec, 0.01
                )  # Draw Axis
    ...
    def render(self, fix: Tuple[int, int]) -> np.ndarray:
        ...
        draw_frame = self.draw_fix(fix, self.container["frame"])
        draw_frame = self.draw_aruco_markers(draw_frame, **self.container["aruco"])
...
class PlanarTracker:
    ...
    def step(self, frame: np.ndarray):
        ...
        corners, ids = self.aruco_tracker.step(frame)

        # Create container
        container = {"frame": frame, "aruco": {"corners": corners, "ids": ids}}

        return PlanarResult(container)
